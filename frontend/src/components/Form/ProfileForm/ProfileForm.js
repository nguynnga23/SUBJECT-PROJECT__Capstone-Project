...
function ProfileForm({
  ...
  const { request: fetchProfile, loading: loadingFetchProfile } = useApi(me);
  const { request: fetchPutProfile, loading: loadingePutProfile } =
    useApi(updateProfile);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (fetched?.ok) {
          setPreCurrentUser(fetched.user);
          setFormData(fetched.user);
  ...
  const handleSave = async () => {
    try {
      const res = await fetchPutProfile({ newProfile: formData });
      if (res) {
        setPreCurrentUser(res);
        console.log(res);
      }
    } catch (error) {
  ...
  return (
    <div className="relative w-[600px] mx-auto bg-white rounded space-y-6 p-6 shadow">
      {loadingFetchProfile || loadingePutProfile ? (
      ...
      <LiaTimesCircle
        className="absolute top-0 right-4 text-gray-500 w-[25px] h-[25px] rounded-full hover:bg-red-500 hover:text-white cursor-pointer"
